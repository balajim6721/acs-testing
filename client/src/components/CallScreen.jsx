  ...
  useEffect(() => {
    const renderStream = async (stream) => {
      ...
      const tryRender = async () => {
        if (!stream.isAvailable || !videoRef.current) return;

        console.log('Stream available:', stream.isAvailable);
      ...
      stream.on('isAvailableChanged', tryRender);
    };
    ...
    const handleVideoStreamsUpdated = (e) => {
      e.added.forEach(stream => renderStream(stream));
      e.removed.forEach(() => {
        if (videoRef.current) {
          videoRef.current.innerHTML = '';
          setHasVideo(false);
    ...
    return () => {
      participant.videoStreams.forEach(stream => {
        stream.off('isAvailableChanged', renderStream);
      });
